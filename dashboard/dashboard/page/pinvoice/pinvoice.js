frappe.pages['pinvoice'].on_page_load = function(wrapper) {

    const page = frappe.ui.make_app_page({
        parent: wrapper,
        title: 'Purchase Invoice Dashboard',
        single_column: true
    });
    ...
    // ─── In-memory stores ─────────────────────────────────────────────────────
    let pi_data   = [];   // Query-1: PI header rows
    let item_data = [];   // Query-2: PI + Item join rows
    ...
    // ─── Title ────────────────────────────────────────────────────────────────
    $(`<div style="padding:18px 20px 0;"><h2>Purchase Invoice Dashboard</h2></div>`)
        .appendTo(page.main);
    ...
    $(`
        <style>
            .pid { padding:18px; background:#f1f5f9; }

            .pid-kpi-row { display:flex; flex-wrap:wrap; gap:14px; margin-bottom:18px; }
            .pid-kpi {
                flex:1 1 160px;
                background:#fff;
                border-radius:10px;
            ...
            .pid-kpi-value { font-size:22px; font-weight:800; color:#0f172a; }
            ...
            .pid-card-head i { color:#6366f1; }
            .pid-card-body { padding:16px 18px; }
            .pid-chart { width:100%; height:300px; }

            .pid-row { display:flex; flex-wrap:wrap; gap:18px; margin-bottom:0; }
            .pid-col-6 { flex:1 1 calc(50% - 9px); min-width:280px; }
        ...
        <div class="pid">
            ...
            <!-- KPIs -->
            <div class="pid-kpi-row" id="pid_kpis"></div>

            <!-- Monthly Trend -->
            <div class="pid-card">
                <div class="pid-card-head"><i class="fa fa-line-chart"></i> Monthly Invoice Trend</div>
                <div class="pid-card-body"><div id="pid_monthly" class="pid-chart"></div></div>
            </div>

            <!-- Row 1 -->
            <div class="pid-row">
                <div class="pid-col-6 pid-card">
                    <div class="pid-card-head"><i class="fa fa-truck"></i> Supplier Outstanding Analysis</div>
                    <div class="pid-card-body"><div id="pid_supplier" class="pid-chart"></div></div>
                </div>
                <div class="pid-col-6 pid-card">
                    <div class="pid-card-head"><i class="fa fa-check-circle"></i> PI Status</div>
                    <div class="pid-card-body"><div id="pid_status" class="pid-chart"></div></div>
                </div>
            </div>
            ...
            <div class="pid-row">
                <div class="pid-col-6 pid-card">
                    <div class="pid-card-head"><i class="fa fa-database"></i> Warehouse Analysis</div>
                    <div class="pid-card-body"><div id="pid_warehouse" class="pid-chart"></div></div>
                </div>
                <div class="pid-col-6 pid-card">
                    <div class="pid-card-head"><i class="fa fa-tags"></i> Item Group Analysis</div>
                    <div class="pid-card-body"><div id="pid_item_group" class="pid-chart"></div></div>
                </div>
            </div>

            <!-- Item Analysis -->
            <div class="pid-card">
                <div class="pid-card-head"><i class="fa fa-cube"></i> Item Analysis</div>
                <div class="pid-card-body"><div id="pid_items" class="pid-chart" style="height:340px;"></div></div>
            </div>
    ...
    // ─── Chart IDs ────────────────────────────────────────────────────────────
    const CHART_IDS = [
        'pid_monthly','pid_supplier','pid_status',
        'pid_warehouse','pid_item_group','pid_items'
    ];
    ...
    function load_data() {
        ...
        let call_pi = new Promise((resolve, reject) => {
            frappe.call({
                method: "dashboard.dashboard.page.pinvoice.pinvoice.get_purchase_invoices",
                args: { filters },
        ...
        let call_items = new Promise((resolve, reject) => {
            frappe.call({
                method: "dashboard.dashboard.page.pinvoice.pinvoice.get_purchase_invoice_items",
                args: { filters },
                callback: r => resolve(r.message || []),
                error: reject
            });
        ...
        Promise.all([call_pi, call_items])
            .then(([pi_rows, item_rows]) => {
                pi_data   = pi_rows;
                item_data = item_rows;

                if (!pi_data.length && !item_data.length) {
                    $('#pid_kpis').html(`
                        <div style="width:100%;">
                ...
                // ── Header aggregations ───────────────────────────────────────
                let total_grand_total = 0, total_outstanding = 0;
                let monthlyMap = {}, supplierMap = {}, statusMap = {};

                pi_data.forEach(row => {
                    let grand_total  = Number(row.grand_total        || 0);
                    let outstanding  = Number(row.outstanding_amount || 0);
                    ...
                    monthlyMap[monthKey] = (monthlyMap[monthKey] || 0) + grand_total;

                    let supp = row.supplier || 'Unassigned';
                    if (!supplierMap[supp]) supplierMap[supp] = { outstanding:0, grand_total:0 };
                    supplierMap[supp].outstanding += outstanding;
                    supplierMap[supp].grand_total += grand_total;
                ...
                render_kpis({ total_pi: pi_data.length, total_grand_total, total_outstanding });
    ...
    function status_badge(status) {
        ...
        if (s === 'paid')                               cls = 'green';
        else if (s === 'unpaid' || s === 'overdue')     cls = 'red';
        else if (s === 'partly paid')                   cls = 'orange';
    ...
    function render_kpis(d) {
        const ACCENT = ['#6366f1','#ef4444','#10b981'];
        let cards = [
            { label:'PI Count',           value: d.total_pi },
            { label:'Outstanding Amount', value: format_currency(d.total_outstanding) },
            { label:'Grand Total',        value: format_currency(d.total_grand_total) },
        ];
        $('#pid_kpis').html(
            cards.map((c, i) => `
                <div class="pid-kpi" style="border-top-color:${ACCENT[i]}">
                    <div class="pid-kpi-label">${c.label}</div>
                    <div class="pid-kpi-value">${c.value}</div>
                </div>
            `).join('')
        );
    }

    // ─── PI-level drill-down (Monthly, Supplier, Status) ──────────────────────
    function open_pi_drill_down(filter_key, clicked_value) {
        let records = pi_data.filter(row => {
        ...
        // FIX: use row.name as the PI docname field (not row.purchase_invoice which is for items)
        ...
        let rows_html = records.length
            ? records.map((row, i) => `
                <tr>
                    ...
                    <td>
                        <a class="dd-link"
                           href="/app/purchase-invoice/${row.name}"
                    ...
                    <td>${row.supplier  || '—'}</td>
                    <td>${row.bill_no   || '—'}</td>
                    <td>${frappe.datetime.str_to_user(row.bill_date) || '—'}</td>
                    <td>${row.company   || '—'}</td>
                    <td class="text-right">${format_currency(row.grand_total,        row.currency)}</td>
                    <td class="text-right">${format_currency(row.outstanding_amount, row.currency)}</td>
        ...
        let dialog_html = `
            ...
            <div class="dd-summary">
                <span><b>${records.length}</b> invoice${records.length !== 1 ? 's' : ''}</span>
                <span>Total: <b>${total_shown}</b></span>
            </div>
        `;

        let modal = new frappe.ui.Dialog({
            title: `Purchase Invoices — ${clicked_value}`,
            size: 'extra-large',
    ...
    function open_pi_item_drill_down(filter_key, clicked_value) {
        ...
        // FIX: use row.parent (standard Frappe child-table field) as the PI name
        // Fall back to row.purchase_invoice if your Python query aliases it that way
        let rows_html = records.length
            ? records.map((row, i) => {
                let pi_name = row.parent || row.purchase_invoice || row.name || '—';
                return `
                <tr>
                    <td style="color:#94a3b8; width:36px;">${i + 1}</td>
                    <td>
                        <a class="dd-link"
                           href="/app/purchase-invoice/${pi_name}"
                           target="_blank">${pi_name}</a>
                    </td>
                    <td>${frappe.datetime.str_to_user(row.posting_date) || '—'}</td>
                    <td>${row.supplier    || '—'}</td>
                    <td>${row.company     || '—'}</td>
            ...
            : `<tr><td colspan="15" style="text-align:center; padding:24px; color:#94a3b8;">No records found.</td></tr>`;
        ...
        let modal = new frappe.ui.Dialog({
            title: `Invoice Items — ${clicked_value}`,
            size: 'extra-large',
            fields: [{ fieldtype: 'HTML', fieldname: 'dd_content' }]
        });
        modal.fields_dict.dd_content.$wrapper.html(dialog_html);
    ...
    function render_monthly_trend(map) {
        ...
        chart.on('click', p => open_pi_drill_down('posting_date', keys[p.dataIndex]));
    }

    // Supplier → PI drill-down
    function render_supplier_chart(map) {
        let supps = Object.keys(map).sort((a,b) => map[b].outstanding - map[a].outstanding).slice(0, 12);
        let chart = ec('pid_supplier');
        chart.setOption({
            tooltip: { trigger:'axis' },
            legend: { data:['Outstanding','Grand Total'] },
            ...
            yAxis: [
                ...
                { type:'value', name:'Amount 2' }
            ],
            series: [
                { name:'Outstanding', type:'bar', data:supps.map(s => map[s].outstanding), itemStyle:{ color:'#ef4444', borderRadius:[4,4,0,0] } },
                { name:'Grand Total', type:'bar', data:supps.map(s => map[s].grand_total), itemStyle:{ color:'#6366f1', borderRadius:[4,4,0,0] } }
            ]
        });
        chart.on('click', p => open_pi_drill_down('supplier', p.name));
    }

    // Status → PI drill-down
    function render_status_chart(map) {
        const COLORS = ['#6366f1','#10b981','#f59e0b','#ef4444','#0ea5e9','#8b5cf6'];
        let chart = ec('pid_status');
        chart.setOption({
    ...
    function render_item_group_chart(map) {
        ...
        chart.on('click', p => open_pi_item_drill_down('item_group', p.name));
    }
    ...
    function render_items_chart(map) {
        ...
        chart.on('click', p => open_pi_item_drill_down('item_code', p.name));
    }
